...
function transform(file, channel) {
  ...
  switch (channel) {
    case 'src':
      relative = path.relative(path.dirname(file.path), path.resolve('demo/miniprogram_npm'));
      break;
    case 'dist':
      relative = '../miniprogram_npm';
      if (/babyfs-wxapp-runningtime/.test(file.path)) return source.toSource();
      break;
    default:
      relative = path.relative(path.dirname(file.path), 'demo/miniprogram_npm');
  }
